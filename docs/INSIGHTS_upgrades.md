...
- **Next signals (the real diversifiers, not yet built):** **quality** (ROE / low debt / earnings stability) and **value** (P/B, P/E, FCF yield) would decorrelate from both ARIMA and momentum — but they hit a **look-ahead trap**: yfinance serves only *current* fundamentals, so they can't be IC-tested on the historical panel without point-in-time data. Honest options: build them **live-screen-only** (current snapshot, no backtest claim) or defer until a PIT fundamentals source exists. Flagged, not silently skipped. **(2026-07-01, Tier-0 decision: DEFERRED to Tier 1 — a paid point-in-time fundamentals feed is the prerequisite; building them on snapshot data would fabricate backtest edge.)**

### U32 — Low-volatility signal ✅ BUILT + MEASURED-ONLY (Tier 0, 2026-07-01)
- **What:** `screener/signals/lowvol_signal.py` — inverse annualized realized vol (252d daily log returns, same formula as `sharpe_signal`), score `1/(1+vol)` so calm names rank high. The low-vol anomaly (Haugen-Baker 1991; Frazzini-Pedersen BAB 2014). Price-only, fully causal → slots into the backtest panel with no look-ahead, unlike value/quality (see U31 note).
- **Status:** measured-only, exactly the momentum discipline — wired into `panel.py` as an always-measured column; the signal-lab IC-tests it automatically (`_signal_keys` auto-discovery); NOT in `EXPECTED_SIGNAL_KEYS`/`WEIGHT_MATRIX`. Promotion requires its IC to earn it. Caveat: expect correlation with Sharpe (shared vol denominator) — check the corr matrix before ever promoting both.
